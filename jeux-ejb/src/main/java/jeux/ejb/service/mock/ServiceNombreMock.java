...
@Stateful
@Remote
public class ServiceNombreMock implements IServiceNombre {
	
	
	// Champs
	
	private static final Logger logger = Logger.getLogger(ServiceNombreMock.class.getName());
	...
	public DtoNombre jouer( String idJoueur, int proposition ) throws ExceptionValidation {
		...
		if ( proposition < 0 || dataNombre.getValeurMaxi() < proposition ) {
			logger.log( Level.WARNING, "Valeur erronée : " + proposition );
